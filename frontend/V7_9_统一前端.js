...
const BUILD={version:'V7',builtAt:'2026-08-05T19:07:54.074470+08:00'};
...
const FULL_TABS=[['chain-overview','产业链总览'],['chain-roadmap','技术与需求路线'],['chain-flow','上下游传导'],['chain-clock','投资时钟'],['chain-companies','公司与板块'],['chain-learn','学习中心']];
const CHAIN_STAGES=[
...
(function () {
  ...
  window.__V7_META__ = { version: "V7.9.1", builtAt: "__V7_BUILD_TIME__" };
  const V74_VALUATION_PENDING = "等待当前估值模型更新";
  const V7_CACHE = {
    temp: new Map(),
    fund: new Map(),
    av: new Map(),
    ...
    allCompanies().forEach((c) => {
      ...
      const liveAction = v73Action(c),
        oneLineCore = String(c.one_liner || "")
          ...
          .replace(/；当前行动：[^。]*。?$/, "");
      c.action = liveAction;
      if (oneLineCore) c.one_liner = `${oneLineCore}；当前行动：${liveAction}。`;
      (c.details || []).forEach((d) => {
  ...
  function v74RenderRefreshCard() {
    ...
    card.innerHTML = `<div class="v74-refresh-main"><div><span class="v74-eyebrow">数据控制中心</span><h2>行情、财报、估值统一刷新</h2><p>在线版后台按四阶段更新；本地HTML只检查已发布快照，并用腾讯实时行情兜底。页面开启时每10分钟检查一次。</p></div><div class="v74-refresh-actions"><button id="v74RefreshNow" class="v74-refresh-now">检查在线数据＋实时行情</button><label class="v74-auto"><input id="v74AutoRefresh" type="checkbox" ${enabled ? "checked" : ""}><span>自动检查</span></label></div></div><div class="v74-market-dates">${market
      .map(([name, key]) => {
        const fresh = D.market_freshness?.[key]?.fresh;
        return `<div class="${fresh === false ? "stale" : "fresh"}"><span>${name}</span><b>${esc(v74MarketDate(key))}</b></div>`;
      })
      .join("")}<div><span>网页上次成功</span><b>${last ? esc(String(last).replace("T", " ").slice(0, 19)) : "尚未手动刷新"}</b></div><div><span>下次检查</span><b>${next ? next.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" }) : "已关闭"}</b></div></div><div id="v74RefreshMessage" class="v74-refresh-message">${D.valuation_meta?.single_source ? "142家公司正在使用同一版估值结果" : "当前为内置快照，建议立即刷新"}</div>`;
    $("#v74RefreshNow").onclick = async () => {
      ...
      try {
        ...
        message.textContent = `刷新成功｜行情截至 ${latestPriceDate()}｜估值 ${D.valuation_meta?.version || "V7.9.1"}`;
      } catch (error) {
        message.textContent = "在线数据暂不可用，继续显示最近一次有效快照";
      } finally {
        button.disabled = false;
        button.textContent = "检查在线数据＋实时行情";
  ...
  function v7InjectUi() {
    document.title = "AI研究系统 V7.9";
    const h = document.querySelector("header .title");
    if (h) h.textContent = "AI研究系统 V7.9";
  ...
  updateStatus = function () {
    ...
    $("#dataBoundary").textContent =
      `当前显示截至 ${ds}；共 ${total} 家（硬件/基础设施 ${D.companies.hardware.length}，应用 ${D.companies.application.length}）。${stale ? "当前为缓存数据。" : ""}`;
  };
  function v76OpenDb() {
    return new Promise((resolve, reject) => {
  ...
  loadCache = function () {
    try {
      ...
      if (
        ...
        /^V7\.9(?:\.1)?$/.test(String(j.valuation_meta?.version || "")) &&
  ...
  refreshAllData = async function (o = {}) {
    ...
    v76RefreshPromise = (async () => {
      ...
      const publicOk = results[0].status === "fulfilled" && results[0].value?.success,
        ...
        success = !!publicOk || quoteCount === totalCompanies();
      if (!success) throw new Error(`完整刷新失败：正式快照${publicOk?'成功':'失败'}，实时行情${quoteCount}/${totalCompanies()}；单家公司更新不计作全量成功`);
      v7ClearCaches();
      populateSector();
      renderCurrent();
      updateStatus();
      ...
      return { success: true, publicOk: !!publicOk, quoteCount, companyOk, results };
    })();
    try {
  ...
  function v73Header() {
    document.title = "AI研究系统 V7.9.1";
    const h = document.querySelector("header .title");
    if (h) h.textContent = "AI研究系统 V7.9.1";
