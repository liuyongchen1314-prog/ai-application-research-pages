...
def strategy_action(
    ...
    price = finite(tech.get("price")) or finite(valuation.get("price_as_of"))
...
def build(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for code, valuation in valuations.items():
        ...
        result[code] = {
            ...
            "reason": reason,
            "valuation_position": valuation_position(finite(tech.get("price")), float(valuation["current_low"]), float(valuation["current_high"])),
